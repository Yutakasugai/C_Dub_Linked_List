...
void addNewNode(struct Node **headNode, int new_data)
{
    // Declare new node
...
int findIndex(struct Node *head_ref, int key)
{
    int index = 0;
    ...
    // Travel the list to find the given value
...
void appendList(struct Node *list1, struct Node *list2)
{
    ...
    // Check if the both lists are empty or not
    ...
    // Travel the first list till the last node
    while (L1->next != NULL)
    {
        L1 = L1->next;
    }

    // Append list2 to the end of the list1
    L1->next = L2;
    L2->prev = L1;
}